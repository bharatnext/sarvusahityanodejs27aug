...
class PostService {
  ...
  async SavePostCategory(postcategroy) {
    var postcategorydata = await this.CategoryRepository.SearchCategory(
      postcategroy
    );
  ...
  async AddPostLike(userInputs) {
    var checkexist = await this.CheckPostIsLikedOrNOt(
      userInputs.user,
      userInputs.post
    );
    if (checkexist == null) {
  ...
  async AddPostWishlist(userInputs) {
    try {
      var checkexist = await this.CheckPostIsWishlistOrNOt(
        userInputs.user,
        userInputs.post
      );
      if (checkexist == null) {
    ...
    try {
      ...
      async function myFunction(PostResult, user) {
        for (const item of PostResult) {
          ...
          var is_wishlist = await EveryPostCheckWishlistOrNot(
            item,
            user,
            myInstance
          );
      ...
      async function EveryPostCheckWishlistOrNot(item, user, myInstance) {
        var checkexist = await myInstance.CheckPostIsWishlistOrNOt(
          user,
          item._id
        );
        if (checkexist == null) {
  ...
  async SearchPost(search, page, size, formdata) {
    try {
      ...
      var PostResult = await this.repository.SearchPosts(
        search,
        page,
        size,
        formdata
  ...
  async SearchForAutocomplete(search, page, size, formdata) {
    try {
      ...
      var PostResult = await this.repository.SearchForAutocomplete(
        search,
        page,
        size,
        formdata
  ...
  async RemovePostLike(userinputs) {
    const PostResult = await this.repository.RemovePostLike(
      userinputs.user,
      userinputs.post
    );
    return [];
  ...
  async RemovePostWishlist(userinputs) {
    const PostResult = await this.repository.RemovePostWishlist(
      userinputs.user,
      userinputs.post
    );
    return [];
  ...
  async ScriptPOST() {
    try {
      ...
      for (const item of PostResult) {
        var categorydata = await this.CategoryRepository.FindCategoryById(
          item.category_id
        );
